        ...
        for (String lib : libids) {
            ...
            try {
                for (OverBook overBook : list) {
                    ...
                    // 查重 distinct会调用equals方法 进行比较
                    List<OverBook> newList = subOverBookList.stream().distinct().collect(Collectors.toList());
                    subOverBookList.clear();
                    subOverBookList.addAll(newList);
